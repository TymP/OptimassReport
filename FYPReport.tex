...
\end{figure}
\section{Calculation of Vapour Surface Velocity}
The vapour surface veloicty is a releavnt parameter to flow meter performance, representing the linear velocity of the vapour phase in the pipe. The VSF depends both on the vapour quality  and the density of the vapour phase, which in turn varies with the saturation conditions.\\
\begin{eqnarray}
...
\section{Overview}
\FloatBarrier
Figures \ref{plot:1} and \ref{plot:2} give an overview of the flow meter in all the conditions tested by plotting relative flow and density errors across a range of vapour quality. The mass flow errors are largely negative, varying from 0-100\% and displaying a non-linear relation to vapour quality. The density errors also vary widely, but their sign changes. The instrument appears to underestimate density in low vapour qualities, and overestimate it at high qualities.
\begin{figure}
...
\label{plot:11}
\end{figure}
Controlling for one of the test conditions thins the data, and has revelaed two interesting trends in the relative mass and density errors. These are discussed below.
\FloatBarrier
...
Nominal Flow Rate & Mass Flux & Vapour Quality & Prected Flow Pattern & Vapour Surface Velocity \\\hline
...
\end{table}
Because flow rate is a first order factor of VSV, this velocity generally increases with nominal flow rate for a given vapour quality, as shown in Figure \ref{gasVelocity}.
%GAS VELOCITY PLOT
\begin{figure}
...
As with the nominal mass flow rate relation, there is no immediate explanation for the temperature trend. It can be speculated that the so called \textit{reduced pressure} plays a role. This possibility will be discussed in \ref{discussion}. 
\begin{figure}
